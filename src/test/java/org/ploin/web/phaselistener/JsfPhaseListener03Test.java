 ...
 * </pre></code>
 * <p/>
 * $HeadURL: https://ploinscm.de/svn/ploinFaces/trunk/src/test/java/org/ploin/web/phaselistener/JsfPhaseListener03Test.java $<br>
 * $LastChangedBy: r.reiz $<br>
 * $Revision: 73 $<br>
 * $Date: 2009-04-27 17:39:05 +0200 (Mon, 27 Apr 2009) $<br>
 */
public class JsfPhaseListener03Test {


	private JsfPhaseListener03 listener;
	...
	@BeforeClass
	public void before(){
		listener = new JsfPhaseListener03();		
	}
	...
	public void getPhaseId(){
		...
		assertEquals(listener.getPhaseId(), PhaseId.PROCESS_VALIDATIONS);
	}
	...
	public void afterPhase(){
		...
		FacesContext fc = new FacesContextMock();
		listener.afterPhase(new PhaseEvent(fc, PhaseId.PROCESS_VALIDATIONS, life));
		assert 1==1;
	}

	/**
	 * In this testcase, we are testing the beforePhase method. <br/>
	 * We expect that the session Attribute jsfPhase is equal to 3.
	 */
	@Test(dependsOnMethods = {"getterAndSetter"})
	public void beforePhase1(){
		Lifecycle life = new LifecycleMock();
		FacesContext fc = listener.getF();
		listener.beforePhase(new PhaseEvent(fc, PhaseId.PROCESS_VALIDATIONS, life));
		HttpSession session = (HttpSession)fc.getExternalContext().getSession(true);
		Integer jp = (Integer)session.getAttribute("jsfPhase");
		assertEquals(jp, new Integer(3));
	}
